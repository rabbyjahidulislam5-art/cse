...
import { QrCode, RefreshCw, Maximize2, Minimize2, Loader2, BookOpen, Copy, Check } from 'lucide-react';
...
export default function LibraryQrPage() {
  ...
  useEffect(() => {
    getLibraryDetails({})
      .then(d => { setLibraryId(d.library.id); setQrToken(d.library.qrToken); setLibraryName(d.library.name); })
      .catch(e => toast.error(e.message))
      .finally(() => setLoading(false));
  }, []);

  const qrPayload = qrToken ? `SMARTCAMPUS:LIBRARY:${libraryId}:${qrToken}` : '';
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await regenerateLibraryQr({});
      ...
      <div className="text-center w-full">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div className={`mx-auto bg-white rounded-2xl shadow-lg flex items-center justify-center overflow-hidden ${kiosk ? 'w-80 h-80' : 'w-64 h-64'}`}>
            {qrImageUrl ? (
              <img src={qrImageUrl} alt="Library QR Code" className={`${kiosk ? 'w-72 h-72' : 'w-56 h-56'}`} />
            ) : (
          ...
          <div className={`mt-4 ${kiosk ? 'mt-8' : ''}`}>
            <div className="flex items-center justify-center gap-2">
              <BookOpen className={`text-primary ${kiosk ? 'w-6 h-6' : 'w-4 h-4'}`} />
              <p className={`font-bold text-foreground ${kiosk ? 'text-3xl' : 'text-lg'}`}>{libraryName}</p>
            </div>
        ...
        <AlertDialogContent className="glass-strong rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will invalidate the current QR code immediately for every Library Staff account. Any printed copies will stop working. Students will need to scan the new QR code.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
