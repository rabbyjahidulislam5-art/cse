...
import { Search, BookMarked, CheckCircle2, XCircle, Clock, Loader2, BadgeCheck, User, FileText } from 'lucide-react';
...
import {
  listAccountsLibraryFines, getAccountsLibraryFineDetail, reconcileLibraryFine,
  type LibraryFineRow, type ListAccountsLibraryFinesOutputType, type AccountsLibraryFineDetailOutputType,
} from '@/lib/api';
import { useNotificationSocket } from '@/lib/socket';
import { FadeIn } from '@/components/PageTransition';
import BackButton from '@/components/BackButton';

const STATUS_TABS = ['All', 'Pending', 'Paid', 'Waived', 'Cancelled'];

function StatChip({ icon: Icon, label, value, color }: { icon: typeof BookMarked; label: string; value: number; color: string }) {
  return (
...
export default function LibraryFinesPage() {
  const [data, setData] = useState<ListAccountsLibraryFinesOutputType | null>(null);
  const [loading, setLoading] = useState(true);
  const [status, setStatus] = useState('All');
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);

  const [detail, setDetail] = useState<AccountsLibraryFineDetailOutputType | null>(null);
  ...
  const load = useCallback(() => {
    setLoading(true);
    listAccountsLibraryFines({ status: status === 'All' ? undefined : status, search: search || undefined, page, pageSize: 20 })
      .then(setData)
      .catch(() => toast.error('Failed to load library fines'))
      .finally(() => setLoading(false));
  }, [status, search, page]);

  useEffect(() => { load(); }, [load]);

  // A library fine is recorded live via notifyRole('Accounts Office', ...) both at issuance and at
  // payment — refetch so this section reflects it immediately, matching the Admin Fines section.
  useNotificationSocket((n) => { if (n.category === 'payment') load(); });

  const openDetail = async (fine: LibraryFineRow) => {
    setDetailOpen(true);
    setDetailLoading(true);
    try {
      const d = await getAccountsLibraryFineDetail({ fineId: fine.id });
      setDetail(d);
  ...
  const handleReconcile = async () => {
    ...
    setReconciling(true);
    try {
      await reconcileLibraryFine({ fineId: detail.fine.id });
      toast.success('Marked as reconciled');
      const refreshed = await getAccountsLibraryFineDetail({ fineId: detail.fine.id });
      setDetail(refreshed);
      load();
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-6 max-w-5xl">
      <FadeIn>
        ...
        <div className="mb-6">
          <h1 className="text-xl font-bold text-foreground">Library Fines</h1>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={0.05}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <StatChip icon={BookMarked} label="Waived" value={counts['Waived'] || 0} color="--chart-2" />
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-5">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder="Search by student, fine type, or reference..."
              aria-label="Search library fines"
      ...
      {(!data || data.fines.length === 0) ? (
        <div className="rounded-2xl border border-border/60 bg-card p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">No library fines match this filter.</p>
        </div>
      ) : (
        <div className="space-y-2">
          <div className="hidden md:grid grid-cols-6 gap-2 px-4 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
            <span className="col-span-2">Student / Fine Type</span><span>Reference</span><span>Amount</span><span>Issued By</span><span className="text-right">Status</span>
          </div>
          {data.fines.map((f, i) => (
            <motion.button
            ...
            >
              <div className="col-span-2">
                ...
                <p className="text-xs text-muted-foreground mt-0.5 truncate">{f.label || f.fineType}</p>
              </div>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="glass-strong rounded-2xl sm:max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Library Fine Detail</DialogTitle></DialogHeader>
          {detailLoading || !detail ? (
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="col-span-2"><p className="text-xs text-muted-foreground">Label</p><p className="font-medium text-foreground mt-0.5">{detail.fine.label || '—'}</p></div>
                <div><p className="text-xs text-muted-foreground">Fine Type</p><p className="font-medium text-foreground mt-0.5">{detail.fine.fineType}</p></div>
                ...
                <div><p className="text-xs text-muted-foreground">Due Date</p><p className="text-foreground mt-0.5">{detail.fine.dueDate || '—'}</p></div>
